    ...
    Object.keys(stack).forEach(category => {
        if (stack[category] && stack[category].length > 0) {
            ...
            categoryDiv.innerHTML = `
                <h5 style="margin-bottom: 6px; color: #666; font-size: 12px; text-transform: uppercase;">
                    ${icon} ${category}
                </h5>
                <div style="font-size: 13px;">
                    ${stack[category].map(item => `<span style="display: inline-block; background: #f0f0f0; padding: 2px 6px; margin: 2px; border-radius: 3px; font-size: 11px;">${item}</span>`).join('')}
                </div>
            `;
